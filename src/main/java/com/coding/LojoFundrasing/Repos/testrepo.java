...
public interface testrepo extends CrudRepository<test, Long> {
	...
	@Query(value = "SELECT * FROM test WHERE committees_id = :committee_id AND testname = :testname AND (varianta = :varianta AND variantb = :variantb OR varianta = :variantb AND variantb = :varianta)", nativeQuery = true)
	...
	@Query(value = "SELECT COUNT(DISTINCT emails.id) FROM emailgroups LEFT JOIN emails on emailgroups.id = emails.emailgroup_id RIGHT JOIN test on emailgroups.test_id = test.id WHERE test.id = :testid AND test.committees_id = :committee_id AND emails.email_name NOT LIKE '%) remainder%'", nativeQuery = true)
	Long testEmailsCount(Long committee_id, Long testid);
	
	@Query(value = "SELECT COUNT(DISTINCT emails.id) FROM emailgroups LEFT JOIN emails on emailgroups.id = emails.emailgroup_id RIGHT JOIN test on emailgroups.test_id = test.id WHERE test.id = :testid AND test.committees_id = :committee_id AND test.varianta = emails.variant AND emails.email_name NOT LIKE '%) remainder%'", nativeQuery = true)
	Long testAEmailsCount(Long committee_id, Long testid);
	
	@Query(value = "SELECT COUNT(DISTINCT emails.id) FROM emailgroups LEFT JOIN emails on emailgroups.id = emails.emailgroup_id RIGHT JOIN test on emailgroups.test_id = test.id WHERE test.id = :testid AND test.committees_id = :committee_id AND test.variantb = emails.variant AND emails.email_name NOT LIKE '%) remainder%'", nativeQuery = true)
	Long testBEmailsCount(Long committee_id, Long testid);
	
	//total recipients calculation
	@Query(value = "SELECT SUM(emails.recipients) FROM emailgroups LEFT JOIN emails on emailgroups.id = emails.emailgroup_id RIGHT JOIN test on emailgroups.test_id = test.id WHERE test.id = :testid AND test.committees_id = :committee_id AND test.varianta = emails.variant AND emails.email_name NOT LIKE '%) remainder%'", nativeQuery = true)
	...
	@Query(value = "SELECT SUM(emails.openers) FROM emailgroups LEFT JOIN emails on emailgroups.id = emails.emailgroup_id RIGHT JOIN test on emailgroups.test_id = test.id WHERE test.id = :testid AND test.committees_id = :committee_id AND test.varianta = emails.variant AND emails.email_name NOT LIKE '%) remainder%'", nativeQuery = true)
	...
	@Query(value = "SELECT SUM(emails.clicks) FROM emailgroups LEFT JOIN emails on emailgroups.id = emails.emailgroup_id RIGHT JOIN test on emailgroups.test_id = test.id WHERE test.id = :testid AND test.committees_id = :committee_id AND test.varianta = emails.variant AND emails.email_name NOT LIKE '%) remainder%'", nativeQuery = true)
	...
	@Query(value = "SELECT test.* FROM emails left join test ON emails.testing = test.testname WHERE test.committees_id = :committee_id AND emails.emaildate >= DATE(:startdateE) and emails.emaildate < DATE_ADD(DATE(:enddateE), INTERVAL 1 DAY) AND testcategory = :operand group by test.id order by test.testname ASC", nativeQuery = true)
	List<test> categoryEquals(@Param("startdateE") @DateTimeFormat(pattern ="yyyy-MM-dd") String startdateE, 
			@Param("enddateE") @DateTimeFormat(pattern ="yyyy-MM-dd") String enddateE, Long committee_id, String operand);
	
	@Query(value = "SELECT test.* FROM emails left join test ON emails.testing = test.testname WHERE test.committees_id = :committee_id AND emails.emaildate >= DATE(:startdateE) and emails.emaildate < DATE_ADD(DATE(:enddateE), INTERVAL 1 DAY) AND testcategory LIKE %:operand% group by test.id order by test.testname ASC", nativeQuery = true)
	List<test> categoryContains(@Param("startdateE") @DateTimeFormat(pattern ="yyyy-MM-dd") String startdateE, 
			@Param("enddateE") @DateTimeFormat(pattern ="yyyy-MM-dd") String enddateE, Long committee_id, String operand);
	
	@Query(value = "SELECT test.* FROM emails left join test ON emails.testing = test.testname WHERE test.committees_id = :committee_id AND emails.emaildate >= DATE(:startdateE) and emails.emaildate < DATE_ADD(DATE(:enddateE), INTERVAL 1 DAY) AND (testcategory IS NULL or testcategory = '' or testcategory = ' ') group by test.id order by test.testname ASC", nativeQuery = true)
	List<test> categoryisBlank(@Param("startdateE") @DateTimeFormat(pattern ="yyyy-MM-dd") String startdateE, 
			@Param("enddateE") @DateTimeFormat(pattern ="yyyy-MM-dd") String enddateE, Long committee_id);
	
	//find tests by test name
	@Query(value = "SELECT test.* FROM emails left join test ON emails.testing = test.testname WHERE test.committees_id = :committee_id AND emails.emaildate >= DATE(:startdateE) and emails.emaildate < DATE_ADD(DATE(:enddateE), INTERVAL 1 DAY) AND testname = :operand group by test.id order by test.testname ASC", nativeQuery = true)
	List<test> testEquals(@Param("startdateE") @DateTimeFormat(pattern ="yyyy-MM-dd") String startdateE, 
			@Param("enddateE") @DateTimeFormat(pattern ="yyyy-MM-dd") String enddateE, Long committee_id, String operand);
	
	@Query(value = "SELECT test.* FROM emails left join test ON emails.testing = test.testname WHERE test.committees_id = :committee_id AND emails.emaildate >= DATE(:startdateE) and emails.emaildate < DATE_ADD(DATE(:enddateE), INTERVAL 1 DAY) AND testname LIKE %:operand% group by test.id order by test.testname ASC", nativeQuery = true)
	List<test> testContains(@Param("startdateE") @DateTimeFormat(pattern ="yyyy-MM-dd") String startdateE, 
			@Param("enddateE") @DateTimeFormat(pattern ="yyyy-MM-dd") String enddateE, Long committee_id, String operand);
	
	@Query(value = "SELECT test.* FROM emails left join test ON emails.testing = test.testname WHERE test.committees_id = :committee_id AND emails.emaildate >= DATE(:startdateE) and emails.emaildate < DATE_ADD(DATE(:enddateE), INTERVAL 1 DAY) AND (testname IS NULL or testname = '' or testname = ' ') group by test.id order by test.testname ASC", nativeQuery = true)
	List<test> testisBlank(@Param("startdateE") @DateTimeFormat(pattern ="yyyy-MM-dd") String startdateE, 
			@Param("enddateE") @DateTimeFormat(pattern ="yyyy-MM-dd") String enddateE, Long committee_id);
}
